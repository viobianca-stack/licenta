...
def s_title(prs, src):
    ...
    txb(sl, cx-Inches(6), cy-Inches(2.5), Inches(12), Inches(0.6),
        "LUCRARE DE DIPLOMA · 2026", size=16, bold=True, color=LGRAY, align=PP_ALIGN.CENTER)
    ...
    txb(sl, cx-Inches(5), cy+Inches(0.7), Inches(10), Inches(1.2),
        "Absolvent: Bianca-Stefania GHEORGHE\nCoordonator: Conf.dr.ing. Daniel-Marian MEREZEANU",
        size=18, color=DARK, align=PP_ALIGN.CENTER)
    txb(sl, cx-Inches(7), cy+Inches(2.4), Inches(14), Inches(0.6),
        "Universitatea Nationala de Stiinta si Tehnologie POLITEHNICA Bucuresti · Facultatea Automatica si Calculatoare",
...
def s_cuprins(prs, src):
    ...
    items = [
        ("01", "Problema si stadiul actual"),
        ("02", "Obiective si tehnologii"),
        ("03", "Arhitectura sistemului"),
        ("04", "Rezultate obtinute"),
        ("05", "Comparatie cu state-of-the-art"),
        ("06", "Contributii si demo aplicatie"),
    ]
...
def s_problema(prs, src):
    ...
    header(sl, "Problema si stadiul actual", 3)
    bullets(sl, Inches(1), Inches(2.3), Inches(6.6), Inches(6),
        [
            ...
            "Retelele sociale = publicare instant, fara filtrare\n→ dezinformare rapida.",
            "Texte scurte si informale: argou, abrevieri, greseli\n→ metodele clasice dau gres.",
            "Instrumentele existente sunt evaluate pe texte\nlungi si formale.",
            "Parafrazarea scade acuratetea de la >90% la <50%.",
    ...
    txb(sl, Inches(8.2), Inches(2.2), Inches(11), Inches(0.5),
        "Peisajul metodelor de detectie (pipeline studiat)", size=15, italic=True, color=LGRAY)
...
def s_obiective(prs, src):
    ...
    items = [
        ("1", "Analiza critica", "Metodele existente si limitarile lor pe social media."),
        ("2", "Solutie specializata", "Detector ML antrenat pe texte scurte (50-500 caractere)."),
        ("3", "Comparatie de modele", "3 clasificatori complementari, conditii identice."),
        ("4", "Aplicatie accesibila", "Interfata web cu justificare transparenta a deciziei."),
    ]
...
def s_tehnologii(prs, src):
    ...
    cats = [
        ...
        ("Aplicatie web", ["Flask 3.x (REST)", "HTML / CSS / JS", "Chart.js"]),
    ]
    ...
    txb(sl, Inches(1.4), Inches(8.8), Inches(17.5), Inches(1.1),
        "Google Colab · GPU NVIDIA Tesla T4 (15.6 GB) · FP16 · RoBERTa ~85 min (3 epoci) · modele clasice <2 min · inferenta CPU 0.5-2 s/text",
        size=16, color=DARK)
...
def s_arhitectura(prs, src):
    ...
    bullets(sl, x+Inches(0.6), Inches(4.85), Inches(6.6), Inches(2.6),
        ["• Filtrate 50-500 caractere",
         ...
         "• 3 modele: Naive Bayes · Reg. Logistica · RoBERTa"], size=17, gap=12)
...
def s_rezultate(prs, src):
    ...
    header(sl, "Rezultate obtinute", 7, "Set de test: 6.876 exemple, echilibrat 50/50")
    # Tabel 4.5
    txb(sl, Inches(1), Inches(2.3), Inches(9), Inches(0.5),
        "Tabel 4.5 — Performanta comparativa", size=16, bold=True, color=BLUE)
    rows = [
        ["Model", "Acuratete", "F1", "Precision", "Recall"],
        ["Naive Bayes", "94.24%", "94.17%", "95.35%", "93.02%"],
        ["Reg. Logistica", "96.71%", "96.66%", "98.11%", "95.26%"],
    ...
    txb(sl, Inches(1), Inches(6.2), Inches(8.7), Inches(2.2),
        "RoBERTa depaseste semnificativ modelele clasice:\n+2.5 puncte fata de Reg. Logistica, +5 puncte fata de Naive Bayes.",
        size=17, color=DARK)
    rect(sl, Inches(1), Inches(7.6), Inches(8.7), Inches(2.4), PANEL)
    bullets(sl, Inches(1.3), Inches(7.85), Inches(8.1), Inches(2.0),
        ["Pe sursele de social media: acuratete > 99.5%",
...
def s_sota(prs, src):
    ...
    header(sl, "Mai bine decat state-of-the-art", 8,
           "Tabel 5.1 — solutia propusa vs. instrumentele comerciale")
    rows = [
        ["Criteriu", "Solutia propusa", "GPTZero", "ZeroGPT", "OpenAI", "Originality", "Sapling"],
        ["Acuratete (social media)", "99.17%", "n/a", "n/a", "26%", "76%", "68-87%"],
        ["Texte scurte <500 car.", "Optimizat", "Slab", "Slab", "F. slab", "Moderat", "Moderat"],
        ["Justificare per cuvant", "Da (LOO)", "Nu", "Nu", "Nu", "Nu", "Nu"],
        ["Comparatie 3 modele", "Da", "Nu", "Nu", "Nu", "Nu", "Nu"],
        ["Scor agregat multi-semnal", "Da", "Nu", "Nu", "Nu", "Nu", "Nu"],
        ["Cost", "Gratuit, open", "Freemium", "Gratuit", "Retras", "Plata", "Gratuit"],
        ["Transparenta metoda", "Cod public", "Partial", "Nu", "Partial", "Nu", "Nu"],
    ]
...
def s_contributii(prs, src):
    sl = clone_slide(src, 7, prs)
    header(sl, "Contributii originale", 9)
    items = [
        ("Set de date dedicat", "6 surse de social media, focalizat pe texte scurte (50-500 car.)."),
        ("Comparatie 3 modele", "NB, Reg. Logistica si RoBERTa in conditii identice."),
        ("Justificare Leave-One-Out", "Explicare la nivel de cuvant — absenta in tool-urile comerciale."),
        ("Scor agregat de risc", "0-100, multi-semnal: RoBERTa 60% + fraze AI 25% + vocabular 15%."),
        ("Aplicatie web integrata", "Analiza, comparatie, NER, per-propozitie, batch CSV — open-source."),
...
def s_demo(prs, src):
    ...
    header(sl, "Demo aplicatie", 10,
           "Verdict + scor de risc · evidentiere cuvinte (Leave-One-Out) · comparatie 3 modele · NER · batch CSV")
...
def s_dezvoltari(prs, src):
    sl = clone_slide(src, 8, prs)
    header(sl, "Dezvoltari viitoare", 11)
    items = [
        ("Extindere multilingva", "XLM-RoBERTa / seturi dedicate, inclusiv limba romana."),
        ("Reantrenare periodica", "Pe modele noi (GPT-4o, Claude, Gemini) — concept drift."),
        ("Robustete la evitare", "Testare impotriva parafrazarii si 'humanizarii'."),
        ("Semnale comportamentale", "Metadate de retea: frecventa postare, profil cont."),
        ("Infrastructura", "Extensie browser, API public REST, Docker."),
        ("Calibrare scor", "Optimizare automata a ponderilor scorului de risc."),
    ]
    for i, (t, d) in enumerate(items):
        col = i % 3; row = i // 3
...
def s_final(prs, src):
    ...
    txb(sl, Inches(1), SH//2-Inches(2.0), Inches(18), Inches(1.5),
        "Va multumesc pentru atentie!", size=46, bold=True, color=BLUE, align=PP_ALIGN.CENTER)
    ...
    txb(sl, Inches(1), SH//2+Inches(0.9), Inches(18), Inches(0.6),
        "Bianca-Stefania GHEORGHE · Automatica si Calculatoare · POLITEHNICA Bucuresti · 2026",
